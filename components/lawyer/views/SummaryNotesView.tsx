...
export const SummaryNotesView: React.FC<SummaryNotesViewProps> = ({
  ...
  // Filter and extract summary notes
  const allNotes = useMemo(() => {
    const list: { caseId: string; clientNames: string; version: string; notes: string; createdBy: string; createdDate: string }[] = [];
    ...
    cases.forEach((c) => {
      ...
      c.notes.forEach((n) => {
        // Enforce Constraint 10
        if (n.visibleTo === activePersona || n.visibleTo === 'BOTH') {
          list.push({
            caseId: c.id,
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1280px]">
      ...
      <div className="bg-emerald-50 border border-emerald-300 rounded-xl p-4 flex items-center gap-3 text-xs text-emerald-800">
        ...
        <div>
          <p className="font-bold">Summary Notes Privacy Active (Constraint #10)</p>
          <p className="text-slate-600 mt-0.5">Showing notes logged for your profile. Opposing counsel's private notes are hidden.</p>
        </div>
      </div>
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-bold font-sans text-slate-900 tracking-tight">
              Confidential Lawyer Notes Summary ({allNotes.length} Found)
            </h3>
        ...
        <table className="w-full text-left text-xs font-sans">
          <thead>
            <tr className="bg-slate-100 text-slate-600 uppercase tracking-wider text-[10px] font-bold border-b border-slate-300">
              ...
              <th className="p-4">Confidential Notes</th>
          ...
          <tbody>
            {allNotes.length > 0 ? (
              allNotes.map((n, idx) => (
                ...
                >
                  <td className="p-4 pl-6 font-mono font-bold text-slate-900">{n.caseId}</td>
                  <td className="p-4 font-semibold">{n.clientNames}</td>
                  <td className="p-4 font-mono font-bold">{n.version}</td>
                  <td className="p-4 italic max-w-[320px] truncate">"{n.notes}"</td>
                  <td className="p-4 font-mono">{n.createdDate}</td>
              ...
              <tr>
                <td colSpan={6} className="p-8 text-center text-slate-400 text-xs font-sans italic">
                  No confidential notes recorded yet for the selected case.
                </td>
              </tr>
            )}
          </tbody>
