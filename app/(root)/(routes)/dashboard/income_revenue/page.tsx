...
export default function IncomeRevenueForm() {
  ...
  const [salaryTreatment, setSalaryTreatment] =
    useState<TreatmentFields>(emptyTreatment);
  ...
  const handlePrimaryBonusToggle = makeToggleHandler(
    setHasPrimaryBonus,
    setPrimaryIncomeRows,
  );
  const handleAltIncomeToggle = makeToggleHandler(
    setHasAlternativeIncome,
    setAltIncomeRows,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await Axios.post(
        `/cases/${caseId}/questionnaire/${user.endUserType === "user1" ? "income-and-revenue" : "partner-income-and-revenue"}`,
        payload,
      );
        ...
        <RowItem key={row.id} onDelete={() => removeRow(setRows, row.id)}>
          <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <input
              ...
              onChange={(e) =>
                updateRow(setRows, row.id, { description: e.target.value })
              }
              required
              className={inputClasses}
            />
            <input
              ...
              value={row.amount}
              onChange={(e) =>
                updateRow(setRows, row.id, { amount: e.target.value })
              }
              required
              className={inputClasses}
            />
          ...
          <TreatmentSelect
            id={row.id}
            fields={row}
            onChange={(f) => updateRow(setRows, row.id, f)}
          />
        </RowItem>
      ))}
    </MatrixBox>
      ...
      <div className="mx-auto max-w-4xl">
        <div className="rounded-2xl bg-white p-11 shadow-[0_10px_25px_-5px_rgba(15,23,42,0.08)]">
          ...
          <p className="mb-8 text-[0.95rem] leading-relaxed text-slate-500">
            Please tell us about all income you personally receive. This
            includes employment income, bonuses, investments, rental income and
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="mb-6">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                What is your current annual gross employment income (before
                tax)?
              </label>
              <input
                type="number"
            ...
            <div className="mb-6">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                How would you like your employment income to be treated under
                this agreement?
              </label>
              <TreatmentSelect
                id="salary"
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                Apart from your base salary, do you regularly receive bonuses,
                commissions, share options, share awards (RSUs), profit-sharing
                payments, or other employment incentives?
              </label>
              <YesNoToggle
                name="has_primary_bonus"
                value={hasPrimaryBonus}
                onChange={handlePrimaryBonusToggle}
              />
            </div>
            {hasPrimaryBonus === "Yes" &&
              renderIncomeSection(
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                Do you personally receive income from sources other than your
                employment, such as rental property income, dividend payments,
                trust distributions, business income, royalties, maintenance
                payments (spousal or child), pension income, or other investment
                income?
              </label>
              <YesNoToggle
