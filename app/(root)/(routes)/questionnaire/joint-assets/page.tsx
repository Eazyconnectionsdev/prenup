        ...
        <form onSubmit={handleSubmit} className="space-y-0">
          <div className="space-y-10 bg-gradient-to-br from-secondary to-primary-foreground text-white py-10 px-6 rounded-lg my-10">
              ...
              <div key={qi} className="flex flex-col items-center text-center pb-6 border-b border-white/20">
                ...
                {qi === 0 && firstQuestion === "yes" && (
                  <div className="mt-6 w-full max-w-3xl text-left bg-white/90 rounded-md p-4">
                    <div className="text-sm text-slate-700 mb-3 font-medium">Please answer the follow-up questions about other shared assets</div>
                    <div className="space-y-4">
                      {FOLLOW_UPS.map((fq, fidx) => (
                        <div key={fidx} className="flex flex-col items-center text-center pb-4 border-b last:border-b-0 border-slate-200">
                          <p className="text-sm text-slate-800 max-w-3xl">{fq}</p>
                          <div className="flex gap-3 mt-3" role="group" aria-label={`Follow up ${fidx + 1}`}>
                            <button
                              type="button"
                              onClick={() => setFollowUpAnswer(fidx, "yes")}
                              aria-pressed={followUps[fidx] === "yes"}
                              className={`px-12 shadow-md hover:bg-gray-100 py-1.5 rounded-full font-medium cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-300 ${followUps[fidx] === "yes" ? "bg-green-600 text-white" : "bg-white text-text-color"}`}
                            >
                              Yes
                            </button>
                            ...
                            <button
                              ...
                              className={`px-12 shadow-md hover:bg-gray-100 py-1.5 rounded-full font-medium cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-300 ${followUps[fidx] === "no" ? "bg-red-600 text-white" : "bg-white text-text-color"}`}
                            >
                              No
                            </button>
                          </div>
            ...
            <footer className="mt-6 text-xs text-slate-200 text-center">
              Answers are saved locally in this demo (check console). I can add localStorage autosave or API integration if you'd like.
            </footer>
          </div>
        </form>
      </div>
