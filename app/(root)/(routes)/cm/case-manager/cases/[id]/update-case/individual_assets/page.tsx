...
function makeSavingsRow(): SavingsRow {
  return { id: makeId("sav"), institution: "", accountType: "", balance: "", ...emptyTreatment };
}
function makePensionRow(): PensionRow {
  return { id: makeId("pen"), provider: "", value: "", valueUnknown: false, ...emptyTreatment };
}
...
function makeChattelRow(): ChattelRow {
  return { id: makeId("chat"), description: "", category: "", value: "", valueUnknown: false, ...emptyTreatment };
}
...
function makeOtherAssetRow(): OtherAssetRow {
  return { id: makeId("other"), description: "", value: "", valueUnknown: false, ...emptyTreatment };
}
  ...
  return (
    <div>
      ...
      <small className="mt-1 block pl-5 text-[0.75rem] leading-tight text-slate-500">
        Please provide an estimated value where possible. You may update this information later if additional details
        become available.
      </small>
    </div>
  );
...
export default function IndividualAssetsForm() {
  ...
  const handleRealEstateToggle = makeToggleHandler(setHasRealEstate, setRealEstate, makeRealEstateRow);
  const handleSavingsToggle = makeToggleHandler(setHasSavings, setSavings, makeSavingsRow);
  const handlePensionsToggle = makeToggleHandler(setHasPensions, setPensions, makePensionRow);
  const handleBusinessesToggle = makeToggleHandler(setHasBusinesses, setBusinesses, makeBusinessRow);
  const handleIPToggle = makeToggleHandler(setHasIP, setIpAssets, makeIPRow);
  const handleChattelsToggle = makeToggleHandler(setHasChattels, setChattels, makeChattelRow);
  const handleOtherAssetsToggle = makeToggleHandler(setHasOtherAssets, setOtherAssets, makeOtherAssetRow);
  ...
  function removeRow<T extends { id: string }>(setRows: React.Dispatch<React.SetStateAction<T[]>>, id: string) {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="rounded-2xl bg-white p-11 shadow-[0_10px_25px_-5px_rgba(15,23,42,0.08)]">
          ...
          <p className="mb-8 text-[0.95rem] leading-relaxed text-slate-500">
            Please tell us about the assets you personally own or partly own. Include any assets owned with another
            person, such as a parent, family member, business partner or trust.
          </p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="mb-4">
              ...
              <YesNoToggle name="has_real_estate" value={hasRealEstate} onChange={handleRealEstateToggle} />
            </div>
            {hasRealEstate === "Yes" && (
              <MatrixBox
                title="Real Estate Registry Asset Rows"
                onAdd={() => setRealEstate((prev) => [...prev, makeRealEstateRow()])}
                addLabel="Add Property Asset"
              >
                {realEstate.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setRealEstate, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                      <input
                        type="text"
                        placeholder="Address Line 1"
                        value={row.addressLine1}
                        onChange={(e) => updateRow(setRealEstate, row.id, { addressLine1: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <input
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { addressLine2: e.target.value })}
                        className={inputClasses}
                      />
                    </div>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-3">
                      <input
                        ...
                        value={row.postcode}
                        onChange={(e) => updateRow(setRealEstate, row.id, { postcode: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <select
                        value={row.propertyType}
                        onChange={(e) => updateRow(setRealEstate, row.id, { propertyType: e.target.value })}
                        required
                        className={inputClasses}
                      >
                      ...
                      <ValueWithUnsure
                        ...
                        onValueChange={(v) => updateRow(setRealEstate, row.id, { value: v })}
                        onUnknownChange={(v) => updateRow(setRealEstate, row.id, { valueUnknown: v })}
                    ...
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-3">
                      <input
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { mortgageBalance: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <input
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { earlyPenalty: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <input
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { ownershipShare: e.target.value })}
                        required
                        className={inputClasses}
                      />
                    </div>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                      <select
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { ownershipMode: e.target.value })}
                        required
                        className={inputClasses}
                      >
                      ...
                      <select
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { thirdPartyInterest: e.target.value })}
                        required
                        className={inputClasses}
                      >
                    ...
                    {row.ownershipMode === "Jointly" && (
                      <textarea
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { coOwnerDetails: e.target.value })}
                    ...
                    {row.thirdPartyInterest === "Yes" && (
                      <textarea
                        ...
                        onChange={(e) => updateRow(setRealEstate, row.id, { thirdPartyDetail: e.target.value })}
                        className={textareaClasses + " mb-3.5"}
                      />
                    )}
                    <TreatmentSelect
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                Do you currently hold any personal bank accounts, cash savings, premium bonds, or investment
                portfolios?
              </label>
              <YesNoToggle name="has_separate_savings" value={hasSavings} onChange={handleSavingsToggle} />
            </div>
            {hasSavings === "Yes" && (
              <MatrixBox
              ...
              >
                {savings.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setSavings, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-3">
                      <input
                        type="text"
                        placeholder="Institution / Bank Name"
                        value={row.institution}
                        onChange={(e) => updateRow(setSavings, row.id, { institution: e.target.value })}
                        required
                        className={inputClasses + " sm:col-span-1"}
                      />
                      <select
                        value={row.accountType}
                        onChange={(e) => updateRow(setSavings, row.id, { accountType: e.target.value })}
                        required
                        className={inputClasses}
                      >
                      ...
                      <input
                        ...
                        value={row.balance}
                        onChange={(e) => updateRow(setSavings, row.id, { balance: e.target.value })}
                        required
                        className={inputClasses}
                      />
                    </div>
                    <TreatmentSelect id={row.id} fields={row} onChange={(f) => updateRow(setSavings, row.id, f)} />
                  </RowItem>
                ))}
              </MatrixBox>
            )}
            ...
            <div className="mb-4">
              ...
              <YesNoToggle name="has_separate_pensions" value={hasPensions} onChange={handlePensionsToggle} />
            </div>
            {hasPensions === "Yes" && (
              <MatrixBox
              ...
              >
                {pensions.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setPensions, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                      <input
                        type="text"
                        placeholder="Pension Provider Name"
                        value={row.provider}
                        onChange={(e) => updateRow(setPensions, row.id, { provider: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <ValueWithUnsure
                        ...
                        value={row.value}
                        unknown={row.valueUnknown}
                        onValueChange={(v) => updateRow(setPensions, row.id, { value: v })}
                        onUnknownChange={(v) => updateRow(setPensions, row.id, { valueUnknown: v })}
                        placeholder="Current CETV Valuation (£)"
                      />
                    </div>
                    <TreatmentSelect id={row.id} fields={row} onChange={(f) => updateRow(setPensions, row.id, f)} />
                  </RowItem>
                ))}
              </MatrixBox>
            )}
            ...
            <div className="mb-4">
              ...
              <YesNoToggle name="has_businesses" value={hasBusinesses} onChange={handleBusinessesToggle} />
            </div>
            {hasBusinesses === "Yes" && (
              <MatrixBox
                title="Business Interests"
                onAdd={() => setBusinesses((prev) => [...prev, makeBusinessRow()])}
                addLabel="Add Corporate Entity"
              >
                {businesses.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setBusinesses, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-[2fr_1fr]">
                      <input
                        type="text"
                        placeholder="Registered Business Name"
                        value={row.name}
                        onChange={(e) => updateRow(setBusinesses, row.id, { name: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <select
                        value={row.entityType}
                        onChange={(e) => updateRow(setBusinesses, row.id, { entityType: e.target.value })}
                        required
                        className={inputClasses}
                      >
                    ...
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-3">
                      <input
                        ...
                        value={row.turnover}
                        onChange={(e) => updateRow(setBusinesses, row.id, { turnover: e.target.value })}
                        className={inputClasses}
                      />
                      <input
                        type="number"
                        ...
                        value={row.netProfit}
                        onChange={(e) => updateRow(setBusinesses, row.id, { netProfit: e.target.value })}
                        className={inputClasses}
                      />
                      <input
                        type="number"
                        ...
                        onChange={(e) => updateRow(setBusinesses, row.id, { ownershipPercent: e.target.value })}
                        required
                        className={inputClasses}
                      />
                    </div>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                      <ValueWithUnsure
                        ...
                        unknown={row.valueUnknown}
                        onValueChange={(v) => updateRow(setBusinesses, row.id, { valueOfStake: v })}
                        onUnknownChange={(v) => updateRow(setBusinesses, row.id, { valueUnknown: v })}
                        placeholder="Value of Stake (£)"
                      />
                      <input
                        type="text"
                        placeholder={
                          row.valueUnknown ? "Not required (Value unknown)" : "Valuation Justification (e.g. Book Value)"
                        }
                        value={row.valueUnknown ? "" : row.justification}
                        disabled={row.valueUnknown}
                        onChange={(e) => updateRow(setBusinesses, row.id, { justification: e.target.value })}
            ...
            <div className="mb-4">
              ...
              <YesNoToggle name="has_ip" value={hasIP} onChange={handleIPToggle} />
            </div>
            {hasIP === "Yes" && (
              <MatrixBox
              ...
              >
                {ipAssets.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setIpAssets, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-[1.5fr_1fr]">
                      <input
                        type="text"
                        placeholder="Intellectual Property Name"
                        value={row.name}
                        onChange={(e) => updateRow(setIpAssets, row.id, { name: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <select
                        value={row.ipType}
                        onChange={(e) => updateRow(setIpAssets, row.id, { ipType: e.target.value })}
                        required
                        className={inputClasses}
                      >
                    ...
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                      <ValueWithUnsure
                        ...
                        value={row.value}
                        unknown={row.valueUnknown}
                        onValueChange={(v) => updateRow(setIpAssets, row.id, { value: v })}
                        onUnknownChange={(v) => updateRow(setIpAssets, row.id, { valueUnknown: v })}
                      ...
                      <input
                        ...
                        onChange={(e) => updateRow(setIpAssets, row.id, { registrationNumber: e.target.value })}
                        className={inputClasses}
                      />
                    </div>
                    <textarea
                      ...
                      onChange={(e) => updateRow(setIpAssets, row.id, { description: e.target.value })}
                      className={textareaClasses + " mb-3.5"}
                    />
                    <TreatmentSelect
                      id={row.id}
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                Do you possess any personal belongings valued individually over £5,000 (such as vehicles, jewelry,
                artwork, or cryptocurrency assets)?
              </label>
              <YesNoToggle name="has_chattels" value={hasChattels} onChange={handleChattelsToggle} />
            </div>
            {hasChattels === "Yes" && (
              <MatrixBox
              ...
              >
                {chattels.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setChattels, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-[2fr_1.5fr_1fr]">
                      <input
                        type="text"
                        placeholder="Asset Description / Name"
                        value={row.description}
                        onChange={(e) => updateRow(setChattels, row.id, { description: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <select
                        value={row.category}
                        onChange={(e) => updateRow(setChattels, row.id, { category: e.target.value })}
                        required
                        className={inputClasses}
                      >
                      ...
                      <ValueWithUnsure
                        ...
                        value={row.value}
                        unknown={row.valueUnknown}
                        onValueChange={(v) => updateRow(setChattels, row.id, { value: v })}
                        onUnknownChange={(v) => updateRow(setChattels, row.id, { valueUnknown: v })}
                        placeholder="Value (GBP)"
                      />
                    </div>
                    <TreatmentSelect id={row.id} fields={row} onChange={(f) => updateRow(setChattels, row.id, f)} />
                  </RowItem>
                ))}
              </MatrixBox>
            )}
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[0.95rem] font-semibold text-slate-800">
                Have we missed anything? Do you own or expect to receive any other assets, financial interests,
                inheritances, or property that have not been listed above?
              </label>
              <YesNoToggle name="has_other_assets" value={hasOtherAssets} onChange={handleOtherAssetsToggle} />
            </div>
            {hasOtherAssets === "Yes" && (
              <MatrixBox
                title="Other Assets Registry"
                onAdd={() => setOtherAssets((prev) => [...prev, makeOtherAssetRow()])}
                addLabel="Add Other Asset"
              >
                {otherAssets.map((row) => (
                  <RowItem key={row.id} onDelete={() => removeRow(setOtherAssets, row.id)}>
                    <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-[2fr_1fr]">
                      <input
                        type="text"
                        placeholder="Asset Name / Description (e.g. Trust Interest, Offshore Account, Safe Deposit Box)"
                        value={row.description}
                        onChange={(e) => updateRow(setOtherAssets, row.id, { description: e.target.value })}
                        required
                        className={inputClasses}
                      />
                      <ValueWithUnsure
                        ...
                        value={row.value}
                        unknown={row.valueUnknown}
                        onValueChange={(v) => updateRow(setOtherAssets, row.id, { value: v })}
                        onUnknownChange={(v) => updateRow(setOtherAssets, row.id, { valueUnknown: v })}
